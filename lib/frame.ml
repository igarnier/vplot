...
let set_axis_options axis options = List.fold_left set_axis_option axis options
...
let set_option frame (opt : options) =
  ...
  | `Caption caption ->
      ...
      { frame with caption }
  | `Xaxis xaxis_opts ->
      { frame with xaxis = set_axis_options default_xaxis xaxis_opts }
  | `Yaxis yaxis_opts ->
      { frame with yaxis = set_axis_options default_yaxis yaxis_opts }
  | `Decoration cmds -> { frame with decorations = frame.decorations @ cmds }

let set_options frame options = List.fold_left set_option frame options
...
let ticked_horizontal_axis (side : [ `Up | `Down ]) origin width xaxis text_size
    tick_labels =
  let tick_num = List.length tick_labels in
  let axis =
    Cmds.segment ~p1:origin ~p2:Pt.(pt (x origin +. width) (y origin))
  in
...
let ticked_vertical_axis (side : [ `Left | `Right ]) origin height yaxis
    text_size tick_labels =
  let tick_num = List.length tick_labels in
  let axis =
    Cmds.segment ~p1:origin ~p2:Pt.(pt (x origin) (y origin +. height))
  in
  let pos_y =
    Utils.interpolate (Pt.y origin) (Pt.y origin +. height) tick_num
  in
  let tick_cmds =
    List.fold_left2
      (fun acc ypos ylab ->
...
let subsample_values values n =
  ...
  if len <= n then Array.to_list values
  else
...
let apply_frame_color frame_color cmd =
  Cmds.style
    ~style:
      Style.(
        make
          ~stroke:(solid_stroke ~clr:frame_color)
          ~width:None
          ~dash:None
          ~fill:None)
    cmd
...
let add_frame frm xvalues yvalues plot =
  ...
  (* adding x label *)
  let xaxis =
    if frm.xaxis.axis_label = "" then xaxis
    else
      let xlblp =
        Pt.(
          pt 0.0 (Units.mm ~-.5.0 :> float) + Cmds.Bbox.(s (of_commands xaxis)))
      in
      let xlbl =
        Cmds.text
  ...
  in
  let yaxis =
    if frm.yaxis.axis_label = "" then yaxis
    else
