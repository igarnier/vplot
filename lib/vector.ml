...
type vector_options = [ `State of state ]

type vector_options_rec = { state : state }

type options = [ vector_options | Frame.options ]

type data = { domain : Vec.t; vecs : vec list }

let default_state () = { min_value = max_float; max_value = ~-.max_float }

let default_options () = { state = default_state () }

let set_option _vector_opt (opt : vector_options) =
  match opt with `State state -> { state }

let parse_options (frame_opt, vector_opt) (option_list : options list) =
  List.fold_left
    (fun (frame_opt, vector_opt) opt ->
      match opt with
      | #Frame.options as o -> (Frame.set_option frame_opt o, vector_opt)
      | #vector_options as o -> (frame_opt, set_option vector_opt o))
    (frame_opt, vector_opt)
    option_list

let get_data = function Full { data; _ } | Simple data -> data

let _get_label = function Full { lab; _ } -> lab | Simple _ -> ""

let _map_data f = function
  | Full { data; sty; lab } -> Full { data = f data; sty; lab }
  | Simple data -> Simple (f data)

let _extract_ticks vec ticks =
  let len = Vec.length vec in
  if ticks < 2 then invalid_arg "extract_ticks: not enough ticks on axis." ;
  if len < ticks then invalid_arg "extract_ticks: too many ticks on axis." ;
...
let subsample data =
  ...
  let rec loop vprev vnow inow vnext acc =
    if inow = len - 2 then (inow + 1) :: inow :: acc
      ...
      let _dot = (vprev -. vnow) *. (vnext -. vnow) in
      (* TODO: normalise length of these vectors*)
      let iprev = inow
      and vprev = vnow
      and vnow = vnext
      and inow = inow + 1
      ...
      else loop vprev vnow inow vnext (iprev :: acc)
  in
  if len = 0 then []
...
let vectors_enveloppe (data : Vec.t list) min_value max_value =
  List.fold_left
    (fun (min_value, max_value) data ->
      let (minv, maxv) = Utils.vector_range data in
      let min = Utils.min min_value minv and max = Utils.max max_value maxv in
      (min, max))
    (min_value, max_value)
    data
...
let draw_curve domain vec =
  ...
  | [] | [_] -> failwith "draw_curve: sample list too short. Bug found."
  | index :: tl ->
...
let plot_internal frame { state } viewport domain data =
  ( match data with
  | [] -> invalid_arg "plot_internal: empty data list on input"
  | _ -> () ) ;
  ...
  if domlen <> datlen then
    invalid_arg
      (Printf.sprintf
         "plot_internal: size mismatch: |domain| = %d, |data| = %d"
         domlen
         datlen)
  else () ;
  let (minv, maxv) =
    let (minv, maxv) =
      vectors_enveloppe points state.min_value state.max_value
    in
...
let plot ~options ~viewport ~data =
  ...
  let (frame, hmap_opts) =
    parse_options (Frame.default, default_options ()) options
  in
