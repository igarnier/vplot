...
type 'a data = { tree : 'a t; tree_type : 'a tree_type }
...
let add_uids (tree : 'a t) =
  ...
  let rec traverse = function
    | `Node (lbl, subtrees) -> `Node (uid lbl, List.map traverse subtrees)
  in
  traverse tree
...
let rec plot_tree params lblfunc (tree : 'a id t) =
  ...
  | `Node ({ elt; id }, subtrees) ->
      ...
      in
      let subnames =
        List.map (function `Node ({ id; _ }, _) -> id) subtrees
      in
      ...
      let nodes =
        Cmds.vbox
          ...
          [Cmds.cmd (Cmds.wrap (sdecl :: ndecl :: cmds)); subplots]
      in
      List.fold_left
        (fun layout id' ->
          Cmds.arrow
...
let list_max (l : float list) =
  let rec loop l res =
    match l with
    | [] -> res
    | hd :: tl -> if hd > res then loop tl hd else loop tl res
  in
  loop l neg_infinity
...
let rec plot_dendrogram params lblfunc (tree : 'a id t) =
  ...
  | `Node ({ elt; id }, subtrees) ->
      ...
      in
      let subplots = List.map (plot_dendrogram params lblfunc) subtrees in
      let subplots = List.map (fun cmds -> Cmds.(cmd (wrap cmds))) subplots in
      ...
      let nodes =
        Cmds.vbox
          ~pos:params.valign
          ~deltay:h
          [Cmds.cmd (Cmds.wrap (sdecl :: ndecl :: lbel)); subplots]
      in
      ...
      let pts =
        List.map
          (function `Node ({ id; _ }, _) -> Cmds.NameMap.find id map)
          subtrees
      in
      ...
      let vbars =
        List.map
          (fun pt -> Cmds.segment ~p1:pt ~p2:(Pt.pt (Pt.x pt) splity))
          pts
      in
      (seg :: hbar :: vbars) @ [cmds]
...
let plot ~options ~viewport ~data =
  ...
  let layout =
    ...
    | Dendrogram { lbl } ->
        Cmds.wrap (plot_dendrogram params lbl (add_uids data.tree))
  in
